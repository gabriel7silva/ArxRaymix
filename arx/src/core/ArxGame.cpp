...
bool ArxGame::initWindow(RenderWindow * window) {
	...
	m_MainWindow->setMaxMSAALevel(config.video.effectiveAntialiasing() ? 8 : 1);
...
static bool g_skipCinematic = false;
static void skipCinematic() {
	g_skipCinematic = true;
}
ARX_PROGRAM_OPTION("skipcinematic", "", "Skip 2D cinematics and letterbox intro speech", &skipCinematic)
